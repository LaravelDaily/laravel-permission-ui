...
@section('content')
    <form class="space-y-4" action="{{ route(config('permissions.route_name_prefix') . 'permissions.update', $permission) }}" method="post">
        @csrf

        <div class="space-y-2">
            <label class="text-base font-medium text-gray-700" for="name">{{ __('PermissionsUI::permissions.permissions.fields.name') }}</label>
            <input class="block w-full rounded-md border border-gray-300 px-2 py-1 shadow-sm" type="text" name="name" id="name" value="{{ old('name', $permission->name) }}" required />
            @error('name')
